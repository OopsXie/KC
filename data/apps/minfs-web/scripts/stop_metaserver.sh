...
# MinFS MetaServer 停止脚本 - 基于IP和端口
# 用法: ./stop_metaServer.sh <host> <port>
...
# 进程锁定机制
LOCK_DIR="/tmp/minfs"
LOCK_FILE="$LOCK_DIR/metaServer_stop_${PORT}.lock"
...
log_step "准备停止 MetaServer ${HOST}:${PORT}..."
...
find_pid() {
    ...
    if [ -f "$PID_FILE" ]; then
        ...
        if [ -n "$pid" ] && [[ "$pid" =~ ^[0-9]+$ ]] && ps -p $pid > /dev/null 2>&1; then
            ...
            if [[ "$cmd_line" == *"metaServer"* ]] || [[ "$cmd_line" == *"MetaServer"* ]]; then
                echo "$pid"
                return
            else
                log_warn "PID文件中的进程不是MetaServer，清理PID文件"
                rm -f "$PID_FILE"
            fi
        else
    ...
    # 4. 尝试通过进程命令行查找
    local pids=$(pgrep -f "(java|MetaServer|metaServer).*$PORT" 2>/dev/null)
    for pid in $pids; do
        if [ -n "$pid" ] && ps -p $pid > /dev/null 2>&1; then
            local cmd_line=$(ps -p $pid -o args= 2>/dev/null)
            if [[ "$cmd_line" == *"metaServer"* ]] && [[ "$cmd_line" == *"$PORT"* ]]; then
                echo "$pid"
                return
            fi
        fi
...
stop_service() {
    ...
    if [ -z "$pid" ]; then
        log_warn "未找到运行中的MetaServer进程"
        return 0
    fi
    
    log_info "正在停止MetaServer进程 (PID: $pid)..."
...
# 主逻辑
log_step "查找MetaServer进程..."

FOUND_PID=$(find_pid)

if [ -n "$FOUND_PID" ]; then
    log_info "找到MetaServer进程: $FOUND_PID"
    
    if stop_service "$FOUND_PID"; then
        log_info "✅ MetaServer ${HOST}:${PORT} 停止成功!"
        
        
        exit 0
    else
        log_error "❌ MetaServer ${HOST}:${PORT} 停止失败!"
        exit 1
    fi
else
    log_warn "未找到运行中的MetaServer进程"
    log_info "✅ MetaServer ${HOST}:${PORT} 已经停止"
    exit 0
fi